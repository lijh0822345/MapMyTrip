...
async function get_formal_city(go_city) {
    // 调用高德地理编码 API
    const response = await fetch(`${(window.API_BASE || '/api/amap')}/v3/geocode/geo?address=${go_city}&output=JSON${keyParam}`);
...
async function getLocation(point, go_city) {
    ...
    // 调用高德地理编码 API
    let url = `${(window.API_BASE || '/api/amap')}/v3/geocode/geo?address=${encodeURIComponent(point)}&output=JSON${keyParam}`;
