...
let city_polygon = null; // 全局变量保存当前显示的图标
...
function addPointMarker(lng, lat) {
  ...
  city_polygon = new ol.layer.Vector({
    ...
    image: new ol.style.Icon({
      src: '/imgs/point_marker.png',   // 自定义图标
      scale: 0.2,           // 图标缩放比例
      anchor: [0.5, 1]    // 设置图片位置
    })
  })
...
function addPolygonFromGaode(keywords) {
  ...
  fetch(`${API_BASE}/v3/config/district?keywords=${encodeURIComponent(keywords)}&subdistrict=0&extensions=all${keyParam}`)
    .then(res => res.json())
    .then(data => {
      if (data.districts && data.districts.length > 0) {
        const polylineStr = data.districts[0].polyline; 
